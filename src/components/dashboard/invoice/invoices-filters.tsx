    ...
    <form onSubmit={handleSubmit(handleApplyFilters)}>
      <Stack spacing={3}>
        <Controller
          ...
          render={({ field }) => (
            <FormControl error={Boolean(errors.id)}>
              <InputLabel>{t('invoicesFilters.invoiceId')}</InputLabel>
              <OutlinedInput {...field} />
            </FormControl>
          )}
        />
        <Controller
          ...
          render={({ field }) => (
            <FormControl error={Boolean(errors.status)} fullWidth>
              <InputLabel required>{t('invoicesFilters.status')}</InputLabel>
              <Select {...field}>
                <Option value="">{t('invoicesFilters.all')}</Option>
                <Option value="pending">{t('invoicesFilters.pending')}</Option>
                <Option value="paid">{t('invoicesFilters.paid')}</Option>
                <Option value="canceled">{t('invoicesFilters.canceled')}</Option>
              </Select>
            </FormControl>
          )}
        />
        <Controller
          ...
          render={({ field }) => (
            <FormControl error={Boolean(errors.customer)}>
              <InputLabel>{t('invoicesFilters.customer')}</InputLabel>
              <OutlinedInput {...field} />
            </FormControl>
          )}
        />
        ...
        <Button disabled={!isDirty} type="submit" variant="contained">
          {t('invoicesFilters.apply')}
        </Button>
        {hasFilters ? (
          <Button color="secondary" onClick={handleClearFilters}>
            {t('invoicesFilters.clearFilters')}
          </Button>
        ) : null}
      </Stack>
    </form>
