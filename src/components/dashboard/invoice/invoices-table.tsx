...
function getGroupTitles(t: (key: string) => string): { canceled: string; paid: string; pending: string } {
  return {
    canceled: t('invoicesFilters.canceled'),
    paid: t('invoicesFilters.paid'),
    pending: t('invoicesFilters.pending'),
  } as const;
}
const columns = [
  {
...
export function InvoicesTable({ rows = [], view = 'group' }: InvoicesTableProps): React.JSX.Element {
  ...
  const groupTitles = getGroupTitles(t);
          ...
          return (
            <Stack key={groupTitles[key]} spacing={2}>
              ...
              {group.length ? (
                ...
                <div>
                  <Typography color="text.secondary" variant="body2">
                    {t('invoicesTable.noInvoicesFound.noInvoicesFound')}
                  </Typography>
                </div>
              )}
            </Stack>
