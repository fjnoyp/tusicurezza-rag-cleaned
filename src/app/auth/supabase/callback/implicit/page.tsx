...
export default function Page(): React.JSX.Element | null {
  ...
  const { t } = useTranslation([LangNs.Auth]);
  ...
  const handle = React.useCallback(async (): Promise<void> => {
    ...
    if (hashParams.get('error')) {
      ...
      setDisplayError(t('error_somethingWrong'));
      return;
    }
    ...
    if (!accessToken || !refreshToken) {
      setDisplayError(t('error_accessTokenOrRefreshTokenMissing'));
      return;
    }
    ...
    if (error) {
      ...
      toast.error(t('error_somethingWrong'));
